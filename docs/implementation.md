...
| `docs/frontend-flow-guide.md` | Step-by-step frontend flow (updated for SSE) |
| `docs/anvil-base-sepolia-e2e.md` | Complete Anvil fork setup guide |
| `docs/deploy-onchain-local-sim.md` | Deploy on Base Sepolia, then run local CRE simulation |
...
- [ ] Push repo to GitHub and set to public
- [ ] Rotate Gemini API key (exposed in git history from earlier commits)
- [ ] Optional: deploy to real Base Sepolia for non-fork demo
- [ ] Optional: run paid x402 proof (402 challenge → buyer retry → 200 success)
